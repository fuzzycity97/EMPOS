...
- **Resolved**: Patient Intake Phone Number Autofill. Added `phone` parameter to `CheckInPatientEvent` and `ClinicBloc._onCheckInPatient`, properly populating `phoneController.text` upon selecting a returning patient in `Autocomplete` and persisting contact details to both patient profiles and CRM customer records.
