...
class OphthalmologyActionWidget extends StatelessWidget {
  ...
  final ValueNotifier<double> cupDiscRatioODNotifier;
  final ValueNotifier<double> cupDiscRatioOSNotifier;
  ...
  OphthalmologyActionWidget({
    ...
    ValueNotifier<double>? cupDiscRatioODNotifier,
    ValueNotifier<double>? cupDiscRatioOSNotifier,
    ValueNotifier<bool>? rightEyeNotifier,
    ValueNotifier<bool>? visualFieldNotifier,
    ValueNotifier<bool>? octScanNotifier,
    this.onApply,
  })  : cupDiscRatioODNotifier = cupDiscRatioODNotifier ?? ValueNotifier<double>(0.40),
        cupDiscRatioOSNotifier = cupDiscRatioOSNotifier ?? ValueNotifier<double>(0.40),
        cupDiscRatioNotifier = cupDiscRatioNotifier ?? ValueNotifier<double>(0.40),
  ...
  ValueNotifier<double> _getActiveRatioNotifier(bool isRight) {
    return isRight ? cupDiscRatioODNotifier : cupDiscRatioOSNotifier;
  }
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            // Visual Simulation of Optic Disc & Cup (Separated by Active Eye)
            ValueListenableBuilder<bool>(
              valueListenable: rightEyeNotifier,
              builder: (context, isRightEye, _) {
                final activeRatioNotifier = _getActiveRatioNotifier(isRightEye);

                return ValueListenableBuilder<double>(
                  valueListenable: activeRatioNotifier,
                      ...
                      children: [
                        ...
                        Center(
                          child: Container(
                            ...
                            child: Text(
                              isGlaucoma
                                  ? '${isRightEye ? "OD (Right)" : "OS (Left)"} • GLAUCOMA SUSPECT / CUPPING (C:D > 0.50)'
                                  : (cdRatio > 0.4
                                      ? '${isRightEye ? "OD (Right)" : "OS (Left)"} • BORDERLINE PHYSIOLOGICAL CUPPING'
                                      : '${isRightEye ? "OD (Right)" : "OS (Left)"} • NORMAL PHYSIOLOGICAL CUP'),
                        ...
                        Row(
                          ...
                          children: [
                            Text(
                              'C:D Ratio Caliper [${isRightEye ? "OD - Right Eye" : "OS - Left Eye"}]',
              ...
              builder: (context, reqVf, _) {
                return Material(
                  ...
                  child: CheckboxListTile(
                    ...
                    subtitle: const Text('Evaluates peripheral visual field defect / arcuate scotoma', style: TextStyle(fontSize: 10)),
                    value: reqVf,
              ...
              builder: (context, reqOct, _) {
                return Material(
                  ...
                  child: CheckboxListTile(
                    ...
                    contentPadding: EdgeInsets.zero,
                    activeColor: const Color(0xFF06B6D4),
                    title: const Text('Spectral-Domain OCT (RNFL & Ganglion Cell Complex)', style: TextStyle(fontSize: 12)),
                    subtitle: const Text('Micron-level retinal nerve fiber layer thickness scan', style: TextStyle(fontSize: 10)),
                    value: reqOct,
            ...
            SizedBox(
              ...
              child: ElevatedButton.icon(
                ...
                onPressed: () {
                  final isRight = rightEyeNotifier.value;
                  final currentRatio = _getActiveRatioNotifier(isRight).value;
                  final annotation = OphthalmologyCupDiscAnnotation(
                    cupToDiscRatio: currentRatio,
                    rightEyeOD: isRight,
